...
if (!File.Exists(filename)) {
    File.WriteAllText(filename, "This is a text file."); //overwrites existing content on file
}
...
using (StreamWriter sw = File.AppendText(filename)){ //opens the file for appended rather than overwriting
    sw.WriteLine("Line 1");
    sw.WriteLine("Line 2");
    sw.WriteLine("Line 3");
}
