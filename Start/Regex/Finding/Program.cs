...
Regex CapWords = new Regex(@"[A-Z]\w+"); //whether there are capitalized words
...
// case-insensitive searches. this is one of the options
// Regex NoCase = new Regex(@"fox", RegexOptions.IgnoreCase);
// Console.WriteLine(NoCase.IsMatch(teststr1));  //true. case is being ignored sucessfully
...
Match m = CapWords.Match(teststr1); //tells us what the capitalized words are 
while (m.Success)
{
